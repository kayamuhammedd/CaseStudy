...
import io
import torch
from diffusers import DiffusionPipeline, DPMSolverMultistepScheduler, StableDiffusionUpscalePipeline, ControlNetModel, StableDiffusionControlNetPipeline
...
class StableDiffusionManager():
        ...
        def ImageGenerate(self, prompt, image, isRemoveBackground):
            if isRemoveBackground:
                ...
                tokenizer = CLIPTokenizer.from_pretrained("laion/CLIP-ViT-H-14-laion2B-s32B-b79K", torch_dtype=torch.float16)
                text_encoder = CLIPTextModel.from_pretrained("laion/CLIP-ViT-H-14-laion2B-s32B-b79K", torch_dtype=torch.float16).to("cuda")
                ...
                tok = tokenizer(prompt, padding="max_length", max_length=tokenizer.model_max_length, truncation=True,
                                return_tensors="pt")
                tok_negative_prompt = tokenizer(negative_prompt, padding="max_length", max_length=tokenizer.model_max_length, truncation=True,
                                return_tensors="pt")
                emb = text_encoder(tok.input_ids.to("cuda"))[0]
                emb_negative = text_encoder(tok_negative_prompt.input_ids.to("cuda"))[0]
                images = pipe(prompt=prompt, negative_prompt=negative_prompt, image=init_image, mask_image=mask_image,
